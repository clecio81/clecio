...
module.exports.run = async (client, message, args) => {
  ...
  const errorEmbed = new Discord.MessageEmbed()
    ...
    .setDescription(
      "Para registrar um usuário, eu preciso da seguinte permissão: **MANAGE_GUILD**"
    );
  if (
  ...
  const member =
    ...
    client.guilds.cache.get(message.guild.id).members.cache.get(args[0]);
  const user =
  ...
  if (!user || user == undefined || user == null) {
    ...
    if (!roles || roles == undefined) {
      ...
      db.add(`usuarios_${message.author.id}_${message.guild.id}`, 1);
